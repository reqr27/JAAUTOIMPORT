...
update Vehiculos set total_invertido_rd = precioRD, total_invertido_usd = precioUSD where vendido = 0

Go

if not exists (select * from TipoTransaccion where id = 8)
	begin
		insert into TipoTransaccion(transaccion)
        Values ('FACTURA SERVICIOS')
	end

Go

delete from CuentasPagar where id_transaccion = 5 or id_transaccion = 6 or id_transaccion = 7

Go
ALTER TABLE GastoVehiculo
...
Go
	...
	alter procedure  [dbo].[insertar_gasto_vehiculo]
...
begin
	...
	if @idTransaccion = 2 -- Compra
		begin 
			...
			as VEHICULO, V.vin as CHASIS, ISNULL(DATEDIFF(DAY, format (Cp.fecha, 'yyyy-MM-dd'), format (GETDATE(), 'yyyy-MM-dd')), 0) as 'DIAS VIGENTE',	
		...
		begin 
			...
			from CuentasPagar CP join Vehiculos V on V.id = CP.id_vehiculo join Fabricantes F on V.fabricante_id = F.id join Modelos M on V.modelo_id = M.id 
			join SeguroVehiculo SV on CP.id_vehiculo = SV.id_vehiculo join Seguros Seg on Seg.id = SV.id_seguro 
...
begin
	if @idTransaccion = 8
		begin
			declare @idFact  int = (Select FS.id from FacturasServicios FS join CuentasPagar CP on CP.id_factura = FS.id where CP.id = @idCP)
			...
			where FC.id_vehiculo = @idVehiculo and  FC.id_factura_servicio = @idFact
		end
	else
		begin
...
begin
			...
			ISNULL(DATEDIFF(DAY, format (v.fecha_importe, 'yyyy-MM-dd'),
			format (GETDATE(), 'yyyy-MM-dd')), 0) as DIASVIGENTE
			from CuentasPagar CP join Vehiculos V on V.id = CP.id_vehiculo join SeguroVehiculo SV on SV.id_vehiculo = CP.id_vehiculo
			join Seguros S on SV.id_seguro = S.id
			join Fabricantes F on F.id = V.fabricante_id join Modelos M on M.id = V.modelo_id
			join TipoTransaccion TP on TP.id = CP.id_transaccion 
...
update CuentasCobrar set fecha = '2017-05-16' where id = 20
update CuentasCobrar set fecha = '2018-03-13' where id = 21
update CuentasCobrar set fecha = '2018-03-13' where id = 22
update CuentasCobrar set fecha = '2018-04-18' where id = 23
update CuentasCobrar set fecha = '2018-03-13' where id = 24
